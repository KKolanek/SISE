...
Get-ChildItem 'Solve\*' | Where-Object { $_.Name -match $SolFilenameRegex } | ForEach-Object {
    $SplitFilename = $_.Name.Split('_');
    $InitFilename = $('4x4\{0}_{1}_{2}.txt' -f $SplitFilename[0], $SplitFilename[1], $SplitFilename[2])
    Write-Host $InitFilename
    Write-Host $('{0}: ' -f $_.Name) -NoNewline
    Invoke-Expression $('{0} {1} {2}' -f $Progcmd, $InitFilename, 'Solve\' + $_.Name)
